...
</callout\> <Badge only>+ v2.9.12</Badge> Gets all Lock Override information from the device.

## Declaration

```cpp
#include "BS_API.h"
int BS2_GetAllLockOverride(void* context, BS2_DEVICE_ID deviceId, BS2LockOverride** responseObj, uint32_t* numOfResponse);
```
...
- \[Out\] `responseObj` : Pointer to receive Lock Override information

- \[Out\] `numOfResponse` : Number of Lock Override information
