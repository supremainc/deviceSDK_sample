...
</callout\> <Badge only>+ v2.9.12</Badge> Sets Lock Override information on the device.

## Declaration

```cpp
#include "BS_API.h"
int BS2_SetLockOverride(void* context, BS2_DEVICE_ID deviceId, const BS2LockOverride* request, uint32_t numOfRequest);
```
...
- \[In\] `request` : Address of the Lock Override structure array to set on the device

- \[In\] `numOfRequest` : Number of structures to set
