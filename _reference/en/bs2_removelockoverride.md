...
</callout\> <Badge only>+ v2.9.12</Badge> Removes Lock Override information from the device by specifying conditions.
...
```cpp
#include "BS_API.h"
int BS2_RemoveLockOverride(void* context, BS2_DEVICE_ID deviceId, const BS2LockOverride* request, uint32_t numOfRequest);
```
...
- \[In\] `request` : Address of the deletion condition structure array

- \[In\] `numOfRequest` : Number of deletion condition structures
