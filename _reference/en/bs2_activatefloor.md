...
Configures the priority of when the door gets activated. The activate priority must be higher than the deactivate to activate the lift.
...
int BS2_ActivateFloor(void* context, uint32_t deviceId, BS2_FLOOR_FLAG flag, BS2_LIFT_ID liftID, uint16_t* floorIndexs, uint8_t floorIndexCount);
```
...
- \[In\] `flag` : Priority of the activate status to add
