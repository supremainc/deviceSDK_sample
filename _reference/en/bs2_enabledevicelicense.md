...
<Badge only>+ v2.9.1</Badge> Activate the device license by selecting the devices to activate the license and the license file.

Slave devices connected to the master device can be collectively selected, and activation results for each device are returned through outResultObj and outNumOfResult.
