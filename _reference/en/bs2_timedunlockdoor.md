...
<Badge only>+ v2.9.12</Badge> Unlocks the door for a specified time, and when the time elapses (timeout), the door restores to its previous state (normalize).

This feature is useful for preventing administrators from omitting door state restoration.

Time can be set in seconds from 1 second to 86,400 seconds (24 hours), and except for the time setting, it is identical to [BS2_UnlockDoor](bs2_unlockdoor).
