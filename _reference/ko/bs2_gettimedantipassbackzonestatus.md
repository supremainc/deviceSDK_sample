# BS2_GetTimedAntiPassbackZoneStatus

주어진 인증 제한 구역의 상태 정보를 가져옵니다.
...
int BS2_GetTimedAntiPassbackZoneStatus(void* context, uint32_t deviceId, uint32_t* zoneIds, uint32_t zoneIdCount, BS2ZoneStatus** zoneStatusObj, uint32_t* numZoneStatus);
```
...
- \[In\] `zoneIds` : 가져오고자 하는 timed anti-passback zone id 리스트

- \[In\] `zoneIdCount` : timed anti-passback zone id 리스트의 크기
