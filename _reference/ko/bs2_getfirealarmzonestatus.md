# BS2_GetFireAlarmZoneStatus

주어진 화재 경보 구역의 상태 정보를 가져옵니다.
...
int BS2_GetFireAlarmZoneStatus(void* context, uint32_t deviceId, uint32_t* zoneIds, uint32_t zoneIdCount, BS2ZoneStatus** zoneStatusObj, uint32_t* numZoneStatus);
```
...
- \[In\] `zoneIds` : 가져오고자 하는 fire alarm zone id 리스트

- \[In\] `zoneIdCount` : fire alarm zone id 리스트의 크기
