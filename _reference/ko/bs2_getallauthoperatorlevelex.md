# BS2_GetAllAuthOperatorLevelEx

<Badge only>+ v2.6.3</Badge> 모든 장치관리자를 가져옵니다.
...
int BS2_GetAllAuthOperatorLevelEx(void* context, BS2_DEVICE_ID deviceId, BS2AuthOperatorLevel** operatorlevelObj, uint32_t* numOperatorlevel);
```

:::info

 [BS2AuthOperatorLevel구조체 보기](device_api#BS2AuthOperatorLevel) 
...
- \[Out\] `operatorlevelObj` : 장치로부터 취득된 관리자들을 저장할 포인터

- \[Out\] `numOperatorlevel` : 장치로부터 취득된 관리자들의 갯수

  **참고**

operatorlevelObj 변수는 사용한 뒤 [BS2_ReleaseObject](bs2_releaseobject) 함수를 이용해 시스템에 메모리를 반환해야 합니다.  
