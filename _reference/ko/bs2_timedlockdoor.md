...
<Badge only>+ v2.9.12</Badge> 지정된 시간 동안 출입문을 잠그며, 시간이 경과(Timeout)하면 출입문은 다시 이전 상태(Normalize)로 복구됩니다.

이 기능은 관리자가 출입문 상태 복원을 누락하는 것을 방지하는 데 유용합니다.

시간은 1초부터 86,400초(24시간)까지 초 단위로 설정할 수 있으며, 시간 설정 외 기능은 [BS2_LockDoor](bs2_lockdoor)와 동일합니다.
